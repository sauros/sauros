...
template <class ContainerKey, class Hash, class Eq> struct RequireUsableKey {
   ...
   std::pair<decltype(std::declval<const Hash &>()(
                 std::declval<const PassedKey &>())),
             decltype(std::declval<const Eq &>()(
                 std::declval<const ContainerKey &>(),
                 std::declval<const PassedKey &>()))> *
 ...
 private:
   ...
   bool has_element(const value_type &elem, size_t hashval) const {
      ...
      while (true) {
         Group g{ctrl_ + seq.offset()};
         for (uint32_t i : g.Match((h2_t)H2(hashval))) {
            if (PHMAP_PREDICT_TRUE(PolicyTraits::element(
                                       slots_ + seq.offset((size_t)i)) == elem))
               return true;
         }
         if (PHMAP_PREDICT_TRUE(g.MatchEmpty()))
            return false;
 ...
 public:
   ...
   bool lazy_emplace_l(const key_arg<K> &key, FExists &&fExists,
      ...
      if (std::get<2>(res))
         ...
         auto it = this->iterator_at(inner,
                                     inner->set_.iterator_at(std::get<1>(res)));
         std::forward<FExists>(fExists)(const_cast<value_type &>(
             *it)); // in case of the set, non "key" part of value_type can be
                    // changed
      }
 ...
 public:
   ...
   bool try_emplace_l(K &&k, F &&f, Args &&...args) {
      ...
      if (std::get<2>(res))
         ...
         auto it = this->iterator_at(inner,
                                     inner->set_.iterator_at(std::get<1>(res)));
         std::forward<F>(f)(const_cast<value_type &>(
             *it)); // in case of the set, non "key" part of value_type can be
                    // changed
      }
      return std::get<2>(res);
