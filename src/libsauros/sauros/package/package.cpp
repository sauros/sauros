...
extern pkg_s load(const std::string &target, sauros::system_c &system, location_s *location,
   ...
   {
      PACKAGE_CHECK(package.env->exists("pkg_name"),
        sauros::format("target: % does not contain the required field `pkg_name`", target)
      )
     ...
     if (package.env->exists("requires")) { 
        ...
        for (auto &required_pkg : requires_cell->list) {
           PACKAGE_CHECK((required_pkg->type == cell_type_e::STRING),
               "package name in pkg's requires list must be of type `string`")
            package.requires_list.push_back(
                required_pkg->data_as_str()
            );
        }
     }
      ...
      if (package.env->exists("library_file")) {
        ...
        PACKAGE_CHECK((package_name_cell->type == cell_type_e::STRING),
...

         {
            ...
            PACKAGE_CHECK((std::filesystem::is_regular_file(library_file_actual)),
                sauros::format("given library_file target: %  for pkg % does not exist", library_file_actual.string(), target))

            package.library_file = library_file_actual.string();
         }

         PACKAGE_CHECK(package.env->exists("library_functions"), 
            sauros::format("% does not contain a library_functions list for library: %",
                target, package.library_file))

         auto library_functions_cell = package.env->get("library_functions");
         
        PACKAGE_CHECK((library_functions_cell->type == cell_type_e::LIST),
         ...
         for (auto &function_name_cell : library_functions_cell->list) {
            ...
            PACKAGE_CHECK((function_name_cell->type == cell_type_e::STRING),
                "function name in pkg's library_functions list must be of type `string`")
      ...
      if (package.env->exists("source_files")) {
        ...
        PACKAGE_CHECK((source_files_cell->type == cell_type_e::LIST),
         ...
         for (auto &file_name_cell : source_files_cell->list) {
            PACKAGE_CHECK((file_name_cell->type == cell_type_e::STRING),
...

            {
               ...
               file_actual /= *file_name_cell->data.s;
                ...
                PACKAGE_CHECK((std::filesystem::is_regular_file(file_actual)),
                    sauros::format("given source_files target: %  for pkg % does not exist", file_actual.string(), target))
