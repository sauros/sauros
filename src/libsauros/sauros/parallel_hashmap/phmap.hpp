...
template <typename T> int TrailingZeros(T x) {
  ...
  else return base_internal::CountTrailingZerosNonZero32(
...
struct GroupPortableImpl {
  ...
  uint32_t CountLeadingEmptyOrDeleted() const {
    ...
    return (uint32_t)((TrailingZeros(((~ctrl & (ctrl >> 7)) | gaps) + 1) + 7) >>
                      3);
  }
...
struct HashtableDebugAccess {
  ...
  // and should increment the result for each non-trivial operation required to
  // find `key`.
  //
...
public:
  ...
  private:
    ...
    void skip_empty_or_deleted() {
      while (IsEmptyOrDeleted(*ctrl_)) {
        ...
        // likely to read past the space for ctrl bytes and into slots. This
        // is ok because ctrl has sizeof() == 1 and slot has sizeof() >= 1
        // so there is no way to read outside the combined slot array.
  ...
  raw_hash_set(raw_hash_set &&that, const allocator_type &a)
      : ctrl_(EmptyGroup()), slots_(nullptr), size_(0), capacity_(0),
    ...
    if (a == that.alloc_ref()) {
      ...
      reserve(that.size());
      // Note: this will copy elements of dense_set and unordered_set instead
      // of moving them. This can be fixed if it ever becomes an issue.
      for (auto &elem : that)
  ...
  PHMAP_ATTRIBUTE_REINITIALIZES void clear() {
    ...
    // compared to destruction of the elements of the container. So we pick
    // the largest bucket_count() threshold for which iteration is still fast
    // and past that we simply deallocate the array.
    if (empty())
      return;
    if (capacity_ > 127) {
      destroy_slots();
  ...
  template <class InputIt,
            typename phmap::enable_if_t<has_difference_operator<InputIt>::value,
                                        int> = 0>
  ...
  // a map, use the following idiom (which also works for standard containers):
  //
...
private:
    ...
    while (true) {
      ...
      for (uint32_t i : g.Match((h2_t)H2(hashval))) {
        ...
        if (PHMAP_PREDICT_TRUE(
                PolicyTraits::apply(EqualElement<K>{key, eq_ref()},
  ...
  bool has_element(const value_type &elem, size_t hashval) const {
    ...
    while (true) {
      ...
      for (uint32_t i : g.Match((h2_t)H2(hashval))) {
        if (PHMAP_PREDICT_TRUE(
                PolicyTraits::element(slots_ + seq.offset((size_t)i)) == elem))
          return true;
      }
      if (PHMAP_PREDICT_TRUE(g.MatchEmpty()))
        return false;
...
public:
  ...
  private:
    iterator(Inner *inner, Inner *inner_end, const EmbeddedIterator &it)
        : inner_(inner), inner_end_(inner_end),
  ...
  parallel_hash_set(std::initializer_list<init_type> init,
                    size_t bucket_cnt = 0, const hasher &hash_param = hasher(),
                    const key_equal &eq = key_equal(),
                    const allocator_type &alloc = allocator_type())
      : parallel_hash_set(init.begin(), init.end(), bucket_cnt, hash_param, eq,
                          alloc) {}
  ...
  emplace_decomposable_with_hash(const K &key, size_t hashval, Args &&...args) {
    Inner &inner = sets_[subidx(hashval)];
    auto &set = inner.set_;
    typename Lockable::UniqueLock m(inner);
    return make_rv(&inner, set.emplace_decomposable(
  ...
  struct EmplaceDecomposableHashval {
    template <class K, class... Args>
    std::pair<iterator, bool> operator()(const K &key, Args &&...args) const {
      return s.emplace_decomposable_with_hash(key, hashval,
                                              std::forward<Args>(args)...);
    }
    parallel_hash_set &s;
    size_t hashval;
  ...
  iterator lazy_emplace_with_hash(const key_arg<K> &key, size_t hashval,
    ...
    return make_iterator(
        &inner, set.lazy_emplace_with_hash(key, hashval, std::forward<F>(f)));
  }
  ...
  // if set contains key, lambda is called with the value_type (under read lock
  // protection), and if_contains returns true. This is a const API and lambda
  ...
  // and lambda should not modify the value This should be used only if we know
  // that no other thread may be mutating the set at the time.
  // -----------------------------------------------------------------------------------------
  template <class K = key_type, class F>
  ...
  bool modify_if(const key_arg<K> &key, F &&f) {
    return modify_if_impl<K, F, typename Lockable::UniqueLock>(
        key, std::forward<F>(f));
  }
  ...
  // erased from the map (the write lock is only released after erase). returns
  // true if key was erased, false otherwise.
  // ----------------------------------------------------------------------------------------------------
  template <class K = key_type, class F>
  bool erase_if(const key_arg<K> &key, F &&f) {
    return erase_if_impl<K, F, typename Lockable::UniqueLock>(
        key, std::forward<F>(f));
  }
  ...
  bool lazy_emplace_l(const key_arg<K> &key, FExists &&fExists,
    ...
    if (std::get<2>(res))
      ...
      std::forward<FExists>(fExists)(
          const_cast<value_type &>(*it)); // in case of the set, non "key" part
                                          // of value_type can be changed
    }
...
protected:
  ...
  find_or_prepare_insert_with_hash(size_t hashval, const K &key,
    ...
    auto p =
        set.find_or_prepare_insert(key, hashval); // std::pair<size_t, bool>
...
template <class K, class V, class Hash, class Eq,
          class Alloc> // default values in phmap_fwd_decl.h
class flat_hash_map
    : public phmap::priv::raw_hash_map<phmap::priv::FlatHashMapPolicy<K, V>,
                                       Hash, Eq, Alloc> {
