...
fs_c::fs_c() {
       ...
       [this](std::vector<cell_c> &cells,
         ...
         if (cells.size() != 1) {
            throw processor_c::runtime_exception_c(
                  "fs::cwd operation expects no parameters",
                  cells[0].location);
         }
         return {sauros::cell_c(sauros::cell_type_e::STRING, std::filesystem::current_path())};
   });
   ...
   _members_map["ls"] = cell_c(
       [this](std::vector<cell_c> &cells,
         ...
         if (cells.size() != 1) {
            throw processor_c::runtime_exception_c(
                  "fs::ls operation expects no parameters",
                  cells[0].location);
         }
         ...
         for (const auto & entry : std::filesystem::directory_iterator(
            ...
            result.list.push_back(
               cell_c(cell_type_e::STRING, entry.path())
            );
         }
         return {result};
   });
   ...
   _members_map["read"] = cell_c(
       [this, load](std::vector<cell_c> &cells,
         ...
         while (std::getline(fs, line)) {
            result.list.push_back(
               cell_c(cell_type_e::STRING, line)
            );
         }
         return result;
   });
   ...
   _members_map["write"] = cell_c(
       [this, load](std::vector<cell_c> &cells,
         ...
         if (item.type != cell_type_e::STRING) {
            throw processor_c::runtime_exception_c(
                  "fs::write first parameter needs to be a string representing a destination",
                  cells[1].location);
         }
   ...
   _members_map["app"] = cell_c(
       [this, load](std::vector<cell_c> &cells,
         ...
         if (item.type != cell_type_e::STRING) {
            throw processor_c::runtime_exception_c(
                  "fs::append first parameter needs to be a string representing a destination",
                  cells[1].location);
         }
