...
math_c::math_c() {
   auto single_arithmetic =
      ...
      for (auto c = cells.begin() + 1; c < cells.end(); ++c) {
         result->list.push_back(std::make_shared<cell_c>(
             cell_type_e::DOUBLE, std::to_string(op((*c), env))));
      }
      return result;
   };

   _members_map["pi"] =
       std::make_shared<cell_c>(cell_type_e::DOUBLE, "3.14159265359");
   _members_map["phi"] =
       std::make_shared<cell_c>(cell_type_e::DOUBLE, "1.61803398875");
   _members_map["e"] =
       std::make_shared<cell_c>(cell_type_e::DOUBLE, "2.71828182845");

   _members_map["log"] = std::make_shared<cell_c>(
       [this, single_arithmetic](
   ...
   _members_map["pow"] = std::make_shared<
      ...
      return std::make_shared<cell_c>(
          cell_type_e::DOUBLE, std::to_string(op(cells[1], cells[2], env)));
   });
}
