...
io_c::io_c() {
   ...
   _members_map["get_str"] = std::make_shared<cell_c>(
       [this](cells_t &cells, std::shared_ptr<environment_c> env) -> cell_ptr {
          std::string line;
          std::getline(std::cin, line);
          return std::make_shared<cell_c>(
              cell_c(sauros::cell_type_e::STRING, line));
       });

   _members_map["get_int"] = std::make_shared<cell_c>(
       [this](cells_t &cells, std::shared_ptr<environment_c> env) -> cell_ptr {
          int64_t x = 0;
          std::string line;
          std::getline(std::cin, line);
   ...
   _members_map["get_double"] = std::make_shared<cell_c>(
       [this](cells_t &cells, std::shared_ptr<environment_c> env) -> cell_ptr {
          double x = 0.0;
          std::string line;
          std::getline(std::cin, line);
