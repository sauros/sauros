...
void processor_c::load_library(const std::string& target,
                                 location_s location,
   ...
   if (!sauros_home.has_value()) {
      throw runtime_exception_c(
         "sauros home directory not found. please set SAUROS_HOME environment variable.",
          location);
   }
   ...
   // std::cout << "looking for : " << target_manifest_file.c_str() << std::endl;
...

   {
      if (!manifest_env->exists("target_name")) {
         throw runtime_exception_c(
            "manifest for target: " + target + " does not contain a target_name",
            location);
      }
      ...
      if (target_name_cell.type != cell_type_e::STRING) {
         throw runtime_exception_c(
            "target_name in manifest for " + target + " is not of type `STRING`",
            location);
      }
      ...
      if (!manifest_env->exists("prefix")) {
         throw runtime_exception_c(
            "manifest for target: " + target + " does not contain a prefix",
            location);
      }
      ...
      if (prefix_cell.type != cell_type_e::STRING) {
         throw runtime_exception_c(
            "prefix in manifest for " + target + " is not of type `STRING`",
            location);
      }
      ...
      if (prefix.empty()) {
         throw runtime_exception_c(
            "prefix in manifest for " + target + " can not be empty",
            location);
      }

      if (!manifest_env->exists("function_list")) {
         throw runtime_exception_c(
            "manifest for target: " + target + " does not contain a function_list",
            location);
      }
      ...
      if (function_list_cell.type != cell_type_e::LIST) {
         throw runtime_exception_c(
            "function_list in manifest for " + target + " is not of type `LIST`",
            location);
      }
   ...
   if (!std::filesystem::is_regular_file(target_manifest_library)) {
      throw runtime_exception_c(
         target_manifest_library.string() + " does not exist within target " + target,
         location);
   }
   ...
   if (!lib->is_loaded()) {
      delete lib;
      throw runtime_exception_c(
         "failed to load library: `" + target_manifest_library.string() + "`",
         location);
   }
   ...
   for(auto &f : function_list) {
      ...
      if (!lib->has_symbol(f)) {
         throw runtime_exception_c(
            "error loading `" + target + "`: listed function `" + f + "` was not found in `" + target_manifest_library.c_str() + "`",
            location);
      }

      void* fn_ptr = lib->get_symbol(f);
      cell_c::proc_f fn = reinterpret_cast<
         std::optional<cell_c>(*)(std::vector<cell_c>&, std::shared_ptr<environment_c>)
      >(fn_ptr); 
