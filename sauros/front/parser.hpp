...
struct product_s {
   ...
   std::shared_ptr<error::error_c> error_info{nullptr}; // Error information, populated iff the an error happened
   cell_c cell;   //! Resulting cell item from parsing
};

//! \brief Parse a line
extern product_s parse_line(
   const char* source_descrption,   //! Description of the source of the line (file, REPL, etc)
