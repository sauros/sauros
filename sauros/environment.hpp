...
public:
   ...
   //! \brief An exception thrown by the request for an identifier that isn't known
   ...
   //! \brief Construct an environment with items auto populated (used for lambdas)
   //! \param params Named parameter objects
   //! \param args The arguments that the parameters represent
   //! \param outer (Optional) environment pointer that will be 
   //!              the "outer" or "upper/parent" environment [super-scope]
   ...
   //! \brief Find the environment that contains the item (current item or a parent super scope)
   //! \param item The item to find
   //! \returns environment pointer that the item can be safely retrieved from using `get`
   //! \note If the item is not reachable within the current, or parent scope(s) 
